        ...
        requestPromise(authXSTSUri, options).then(response => {
            if (response.body.XErr) {
                switch (response.body.XErr) {
                    case 2148916233:
                        reject({
                            message: 'Votre Compte microsoft n\'est pas relier a un compte Xbox!.<br>'
                        })
                        return

                    case 2148916238: 
                        reject({
                            message: 'Puisque vous avez pas encore 18 ans, Nous vous invitons à demander l\'accord parental avant d\'utiliser le Palifen Launcher!'
                        })
                        return
